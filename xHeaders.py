import requests , os , psutil , sys , jwt , pickle , json , binascii , time , urllib3 , base64 , datetime , re ,socket , threading , asyncio
...
async def equie_emote(JWT,url):
    ...
    try:
        response = await asyncio.to_thread(requests.post, url, headers=headers, data=data, timeout=10)
        return response.status_code == 200
    except Exception as e:
        print(f"Emote API Error: {e}")
        return False
